...
export async function buildRecipientSet(
  ...
  for (const email of candidates) {
    const h = await emailHash(secret, email);
    if (!suppressedHashes.has(h)) result.push(email);
  }
  return result.sort();
}
...
export function parseCcArticleLinks(xml: string, limit = 5): CcArticleLink[] {
  ...
  for (const m of xml.matchAll(/<item\b[^>]*>([\s\S]*?)<\/item>/gi)) {
    const block = m[1];
    const isCc =
      /claude[\s-]?code/i.test(block) || /\/claude-code\//i.test(block);
    if (!isCc) continue;

    const title = stripCdata(block.match(/<title\b[^>]*>([\s\S]*?)<\/title>/i)?.[1] ?? "").trim();
    const url = stripCdata(block.match(/<link\b[^>]*>([\s\S]*?)<\/link>/i)?.[1] ?? "").trim();
